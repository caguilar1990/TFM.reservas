(function($) {
	...
	function refreshClass() {
		...
		if ($("#estadoSelect").val() == "") {
			...
			var deferred = bookingService.findClassroomsBookingByDateMaster($("#fecha").val(),IDPersonal);
			deferred.done(showClassrooms).always(function() {
			});

		} else {
			// Busqueda por Estado
			var deferred = bookingService.findClassroomsBookingByDateAndStateMaster($("#fecha").val(), $("#estadoSelect").val(),IDPersonal);
			deferred.done(showClassrooms).always(function() {
			});
		}
